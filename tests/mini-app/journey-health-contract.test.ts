...
describe("NimCarry 1→5 journey health contract", () => {
  ...
  it("uses the bridge once and completes at the destination after FINAL", () => {
    ...
    expect(app).toContain("Accepted — waiting for the payment");
    expect(app).toContain("received the 1 NIM. Your part is done.");
  ...
  it("renders the bridge once as via, never as a new holder after recovery", () => {
    ...
    expect(app).toContain("`Introduced by ${bridgeMark}`");
    expect(app).not.toContain("You now carry this letter");
  });
